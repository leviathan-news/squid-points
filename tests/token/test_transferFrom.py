...
def test_caller_approval_affected(accounts, token):
    ...
    token.approve(accounts[1], approval_amount, sender = accounts[0])
    token.transferFrom(accounts[0], accounts[2], transfer_amount, sender = accounts[1])
...
def test_receiver_approval_not_affected(accounts, token):
    ...
    token.approve(accounts[1], approval_amount, sender = accounts[0])
    token.approve(accounts[2], approval_amount, sender = accounts[0])
    token.transferFrom(accounts[0], accounts[2], transfer_amount, sender = accounts[1])
...
def test_returns_true(accounts, token):
    amount = token.balanceOf(accounts[0])
    token.approve(accounts[1], amount, sender = accounts[0])
    tx = token.transferFrom(accounts[0], accounts[2], amount, sender = accounts[1])
...
def test_transfer_zero_tokens(accounts, token):
    ...
    token.approve(accounts[1], sender_balance, sender = accounts[0])
    token.transferFrom(accounts[0], accounts[2], 0, sender = accounts[1])
...
def test_insufficient_approval(accounts, token):
    ...
    token.approve(accounts[1], balance - 1, sender = accounts[0])
    with ape.reverts():
        token.transferFrom(accounts[0], accounts[2], balance, sender = accounts[1])
...
def test_revoked_approval(accounts, token):
    ...
    token.approve(accounts[1], balance, sender = accounts[0])
    token.approve(accounts[1], 0, sender = accounts[0])
...
def test_transfer_to_self(accounts, token):
    ...
    token.approve(accounts[0], sender_balance, sender = accounts[0])
    token.transferFrom(accounts[0], accounts[0], amount, sender = accounts[0])
